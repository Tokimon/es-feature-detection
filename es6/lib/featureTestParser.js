        ...
        (
          (res &&
            (
              ...
              (
                ((res.method && methods.has(`${res.object}.${res.method}`)) || !res.method)
              )
            )
          ) ||
          !res
...
function composeFeatureFile(features, filePath) {
  if(!filePath) { filePath = 'featuretest.js'; }

  return composeFeatures(features)
    .then((composedScript) => new Promise((resolve, reject) => {
      mkdirp(nPath.dirname(filePath), (err) => {
        if(err) { reject(err); }

        const writeStream = fs.createWriteStream(filePath);
        writeStream.write(`const features = ['${composedScript.features.join("','")}'];\n`);
...
module.exports.write = composeFeatureFile;
