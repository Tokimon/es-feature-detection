...
const features = require('../browser/features')(this);
...
// Map the features from the feature detection
function mapFeatures() {
  // Don't map the features again if they have already been found since the file won't change i between
  if(methods.size && objects.size) { return; }

  // Only add objects that doesn't have method names specified
  // - this should mean that the object shuld just be checked for existance
  Object.keys(features).filter((name) => {
    const obj = features[name];
    if(typeof obj === 'function') { return objects.add(name); }

    // Add the methods to the methods Map
    Object.keys(obj).forEach((method) => {
      if(method === name) { return objects.add(method); }
      const objNames = methods.get(method) || [];
      objNames.push(name);
      methods.set(method, objNames);
    });
  }, {});
}
